/* programs.jsx — Programs index + per-program detail view */

const Programs = ({ activeId, setActiveId, onEnquire, setPage }) => {
  if (activeId) {
    const p = window.PROGRAMS.find((x) => x.id === activeId);
...
const ProgramsIndex = ({ setActiveId, onEnquire, setPage }) => {
  ...
  const filtered = window.PROGRAMS.filter((p) => {
  ...
  return (
    <main>
      ...
      {/* List of programs as editorial blocks */}
