/* common.jsx — shared components: Nav, Footer, ImagePlaceholder, etc. */

const Wordmark = ({ size = 'md', oneline = false }) => {
  const sizeMap = {
...
const Nav = ({ page, setPage, onEnquire }) => {
      ...
      {mobileOpen && (
        <div className="mobile-drawer">
          ...
          <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 12, paddingTop: 24 }}>
            ...
            <a href="https://instagram.com/swaswasthya" target="_blank" className="mono" style={{ textAlign: 'center', color: 'var(--ink-soft)', letterSpacing: '0.14em', textTransform: 'uppercase', fontSize: 11, paddingTop: 4 }}>
              @swaswasthya
            </a>
          </div>
        </div>
...
/* In-page theme switcher — ships inside the design so anyone who
   opens the shared HTML can try the palettes themselves. */
const THEMES = [
...
const ThemeSwitch = ({ palette, setPalette }) => {
...
const Footer = ({ setPage, onEnquire }) => (
  <footer style={{
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 14, color: 'rgba(240, 229, 207, 0.85)' }}>
            ...
            <a href="https://instagram.com/swaswasthya" target="_blank">@swaswasthya</a>
...
// Image placeholder with monospaced label
const ImgPh = ({ label, height = 320, radius = 'var(--radius-md)', style = {} }) => (
  <div className="imgph" style={{ height, borderRadius: radius, ...style }}>
    <div className="imgph-label">▣ {label}</div>
  </div>
);

// Section number + label header
...
// Marquee with editorial words
const Marquee = ({ items, italic = true }) => (
...
// Decorative arc / sun motif (used as accent)
const SunArc = ({ size = 200, color = 'var(--clay)' }) => (
...
Object.assign(window, { Wordmark, Logo, Nav, Footer, ImgPh, SectionLabel, Marquee, SunArc, ThemeSwitch });
