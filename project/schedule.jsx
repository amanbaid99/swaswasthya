/* schedule.jsx — Weekly schedule with filtering */

const Schedule = ({ onEnquire }) => {
  const [view, setView] = React.useState('week'); // week | list
  const [mode, setMode] = React.useState('All'); // All | Online | Offline
  const [program, setProgram] = React.useState('All');

  const programs = ['All', ...window.PROGRAMS.map((p) => p.name.replace('The ', '').replace(' Edit', ''))];

  // Build a schedule matrix
  const sessions = [
    ...
    // Tej / Dhyana / Antara — placeholder slots
  ...
  return (
    <main>
      ...
      <section style={{ padding: '48px 0 80px' }}>
        <div className="container">
          {view === 'week' ? (
            <WeekGrid sessions={filtered} days={days} timeSlots={timeSlots} programColors={programColors} onEnquire={onEnquire} />
          ) : (
            <ListView sessions={filtered} programColors={programColors} onEnquire={onEnquire} />
          )}
...
const WeekGrid = ({ sessions, days, timeSlots, programColors, onEnquire }) => {
  return (
    <div>
      <style>{`
      ...
      <div style={{
        ...
        {timeSlots.map((slot, slotIdx) => (
          <React.Fragment key={slot}>
            ...
            {days.map((day) => {
              const s = sessions.find((x) => x.day === day && x.time === slot);
              return (
                <div key={day + slot} style={{
                  ...
                  {s && <SessionChip s={s} programColors={programColors} onClick={onEnquire} />}
                </div>
              );
            })}
          </React.Fragment>
...
const SessionChip = ({ s, programColors, onClick }) => {
...
const ListView = ({ sessions, programColors, onEnquire }) => {
